...
export default function App() {
  ...
  const getPermissions = async () => {
    ...
    let storageStatus;
    if (Platform.OS === 'ios') {
      storageStatus = (await Permissions.askAsync(Permissions.MEDIA_LIBRARY)).status;
    } else {
      storageStatus = (await Permissions.askAsync(Permissions.WRITE_EXTERNAL_STORAGE)).status;
    }
  ...
  async function shareRecording() {
    try {
      ...
      Alert.alert('Share Failed', 'An error occurred while trying to share the recording.');
    }
  }
